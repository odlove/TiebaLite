...
public final class ColorStateListUtils {
    ...
    public static ColorStateList createColorStateList(Context context, int resId) {
        ...
        if (value.type >= TypedValue.TYPE_FIRST_COLOR_INT && value.type <= TypedValue.TYPE_LAST_COLOR_INT) {
            cl = ColorStateList.valueOf(ThemeColorResolver.colorById(context, value.resourceId));
        } else {
            ...
            if (file.endsWith("xml")) {
                try (XmlResourceParser parser = context.getResources().getAssets().openXmlResourceParser(value.assetCookie, file)) {
    ...
    private static ColorStateList createFromXmlInner(Context context, XmlPullParser parser, AttributeSet attrs) throws IOException, XmlPullParserException {
        if (!"selector".equals(parser.getName())) {
            throw new XmlPullParserException(parser.getPositionDescription() + ": invalid color state list tag " + parser.getName());
        }
    ...
    private static ColorStateList inflateColorStateList(Context context, XmlPullParser parser, AttributeSet attrs) throws IOException, XmlPullParserException {
        ...
        while ((type = parser.next()) != XmlPullParser.END_DOCUMENT && ((depth = parser.getDepth()) >= innerDepth || type != XmlPullParser.END_TAG)) {
            ...
            final int baseColor = value == android.graphics.Color.MAGENTA
                    ...
                    : ThemeColorResolver.colorById(context, value);
            a1.recycle();
            ...
            int finalColor = alphaMod != 1.0f
                    ? ColorUtils.setAlphaComponent(baseColor, Math.round(android.graphics.Color.alpha(baseColor) * alphaMod))
                    : baseColor;
