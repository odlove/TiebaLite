package com.huanchengfly.tieba.core.network.http
...
inline fun Array<out ParamExpression>.forEachNonNull(action: (String, String) -> Unit) {
...
inline fun List<ParamExpression>.forEachNonNull(action: (String, String) -> Unit) {
    forEach { (name, valueExpression) ->
        val value = valueExpression()
        if (value != null) {
            action(name, value)
