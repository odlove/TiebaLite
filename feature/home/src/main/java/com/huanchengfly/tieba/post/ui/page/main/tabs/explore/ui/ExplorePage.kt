package com.huanchengfly.tieba.post.ui.page.main.tabs.explore.ui
                    ...
                    coroutineScope.launch {
                        if (pagerState.currentPage == index) {
                            coroutineScope.emitGlobalEvent(
                                globalEventBus,
                                CommonUiEvent.Refresh(pages[index].id)
                            )
                        } else {
    ...
    val pages = remember(loggedIn) {
        listOfNotNull(
            if (loggedIn) ExplorePageItem(
                "concern",
                { TabText(text = stringResource(id = CoreUiR.string.title_concern), selected = it) },
                { ConcernPage() }
            ) else null,
            ExplorePageItem(
                "personalized",
                { TabText(text = stringResource(id = CoreUiR.string.title_personalized), selected = it) },
                { PersonalizedPage() }
            ),
            ExplorePageItem(
                "hot",
                { TabText(text = stringResource(id = CoreUiR.string.title_hot), selected = it) },
                { HotPage() }
            ),
        ).toImmutableList()
    }
        ...
        topBar = {
            Column(
                ...
                ThemeTopAppBar(
                    title = {
                        Text(
                            text = stringResource(id = CoreUiR.string.title_explore),
